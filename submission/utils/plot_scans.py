...
def plot_scans(frames,scan_name,label_GP,pred_GP,saved_folder,tform_calib_scale,image_points):
    # plot scan in 3D
...
def plotting(gt,frame,axs,step,color,width = 4, scatter = 8, legend_size=50,legend = None): 
    ...
    for i,ax in enumerate(axs):
        ...
        ax.plot(gt[-1,0,0:2], gt[-1,1,0:2], gt[-1,2,0:2], c = color, linewidth = width)
        ax.plot(gt[-1,0,[1,3]], gt[-1,1,[1,3]], gt[-1,2,[1,3]], c = color, linewidth = width) 
        ax.plot(gt[-1,0,[3,2]], gt[-1,1,[3,2]], gt[-1,2,[3,2]], c = color, linewidth = width) 
        ax.plot(gt[-1,0,[2,0]], gt[-1,1,[2,0]], gt[-1,2,[2,0]], c = color, linewidth = width)
