...
from utils.transform import LabelTransform, PredictionTransform,Transforms,TransformAccumulation
...
class Evaluation():  
    ...
    def __init__(self, opt,device, dset,model_name,saved_folder):
        self.opt = opt
        self.device = device
        self.dset = dset
        ...
        self.transform_to_transform = Transforms(
    ...
    def cal_label_globle_allpts(self,frames,tforms,tforms_inv):
        ...
        data_pairs_all = data_pairs_global(frames.shape[1])[1:,:]
    ...
    def scan_plot(self,scan_index):
        ...
        plot_scan(labels_four,frames,os.path.join(saved_img_path,'sub%03d__%s' % (indices[0],scan_name)+'_label_1'),step = frames.shape[0]-1,color = color[0],width = 4, scatter = 8, legend_size=50, legend = 'GT')
        plot_scan(pred_four,frames,os.path.join(saved_img_path,'sub%03d__%s' % (indices[0],scan_name)+'_pred_1'),step = frames.shape[0]-1,color = color[1],width = 4, scatter = 8, legend_size=50, legend = 'Pred')
        # plot label and prediction in the same figure 
        plot_scan_label_pred(labels_four,pred_four,frames,color,os.path.join(saved_img_path,'sub%03d__%s' % (indices[0],scan_name)+'_pred_label_1'),step = frames.shape[0]-1,width = 4, scatter = 8, legend_size=50)
